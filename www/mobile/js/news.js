...
var app = angular.module('yz', ["ngRoute", "ngSanitize"])
	...
	.controller("newsController", function($scope, $window, toastServices, errorServices, newsServices) {
		...
		$scope.show_detail = function(id) {
			...
			$window.location.href = "mobile/new.html?information_id=" + id;
		}
	})
	.controller("newController", function($scope, $location, toastServices, errorServices, newsServices) {
		toastServices.show();
		newsServices.queryById({
			information_id: $location.search().information_id
		}).then(function(data) {
			$("body").css({
				visibility: "visible"
			});
			toastServices.hide()
			if (data.code == config.request.SUCCESS && data.status == config.response.SUCCESS) {
				$scope.new = data.Result.Information;
			} else {
	...
	.factory("newsServices", function($http) {
		return {
			...
			queryById: function(input) {
				return $http({
					// by dribehance <dribehance.kksdapp.com>
					url: config.url + "/app/Informations/Information",
					method: "GET",
					params: angular.extend({}, config.common_params, input)
				}).then(function(data) {
					return data.data;
	...
	.directive('imageview', function() {
		return {
			...
			link: function(scope, element, attrs) {
				var bg_image = "../mobile/images/zwpic.png";
